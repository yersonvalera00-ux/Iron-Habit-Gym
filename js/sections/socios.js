...
async function handleSocioSubmit(event) {
  ...
  const btnGuardar = document.getElementById("btn-save-socio");
  ...
  const peso       = document.getElementById("socio-peso").value;
  const estatura   = document.getElementById("socio-estatura").value;
  const grasa      = document.getElementById("socio-grasa").value;
  const musculo    = document.getElementById("socio-musculo").value;
  const antecedentes = document.getElementById("socio-antecedentes").value.trim() || "";

  // Preparamos los datos para enviar al servidor PHP
  ...
  try {
    // ── Enviamos al servidor PHP con fetch ──
    ...
    if (resultado.exito) {
      // También guardamos en localStorage para que la lista se actualice sin recargar
      const nuevoId = resultado.id;
      const nuevoSocio = { id: nuevoId, nombre, telefono, email, membresiaId, fecha, estado };
      socios.push(nuevoSocio);
      fichasCorporales[nuevoId] = { peso, estatura, grasa, musculo, antecedentes };

      actualizarSQLConsola("INSERT_SOCIO", { ...nuevoSocio, ficha: fichasCorporales[nuevoId] });
      guardarBD();

      mostrarToast("✓ Socio guardado en la base de datos MySQL.", "success");
      limpiarAutosave(AUTOSAVE_KEY_SOCIO);
      resetSocioForm();
      renderizarListaSocios();
    } else {
      mostrarToast("Error del servidor: " + resultado.mensaje, "error");
    }

  } catch (error) {
    // Si fetch falla (sin servidor, CORS, etc.) informamos claramente
    mostrarToast("No se pudo conectar con el servidor. Verifica que XAMPP esté corriendo.", "error");
    console.error("Error fetch guardar socio:", error);
  } finally {
